...
class ComputationAwareELBO(MarginalLogLikelihood):
    ...
    def __init__(
        ...
        model: "models.ComputationAwareGP",
    ):
    ...
    def forward(self, outputs: torch.Tensor, targets: torch.Tensor, **kwargs): # type: ignore[reportOptionalSubscript, union-attr]
        ...
        # Kernel: determine whether we can use the efficient KeOps path
        ...
        use_keops = hasattr(base_kernel, '_forward_no_kernel_linop')
        ...
        if use_keops:
            # Efficient KeOps path: kernel returns raw lazy tensor (no lengthscale/ outputscale applied)
            K_lazy = base_kernel._forward_no_kernel_linop( # type: ignore[reportOptionalSubscript, union-attr]
            ...
            # Fallback: standard gpytorch kernel evaluation (less memory efficient).
            # The kernel handles lengthscale and outputscale internally, so set outputscale=1
            # to avoid double-counting it downstream.
            outputscale = 1.0
            P = self.model.projection_dim
            N = self.model.num_non_zero
            K_lazy = self.model.covar_module( # type: ignore[reportOptionalSubscript, union-attr]
                train_inputs.view(1, P, N, train_inputs.shape[-1]),
                train_inputs.view(P, 1, N, train_inputs.shape[-1]),
            )

        # Compute S'K in block shape (PROJ_DIM, PROJ_DIM, NNZ)
        StK_block_shape = ( # type: ignore[reportOptionalSubscript, union-attr]
            K_lazy @ self.model.actions_op.blocks.view(self.model.projection_dim, 1, self.model.num_non_zero, 1) # type: ignore[reportOptionalSubscript, union-attr]
        ).squeeze(-1)
