...
def seed_check(seed: int | None, os_scale_training: bool, dset: RegressionDataset, gptype: object, opt_str: str, approx_size: int, kernel_name: str, inducing_point_method: str | None = None) -> bool:
    ...
    scale training flag, optimizer name, approximation size, kernel and
    ...
    Args:
        ...
        kernel_name: Kernel display name recorded in results. Distinguishes
            otherwise identical runs that differ only in kernel (e.g. ``RBF``
            vs ``RBFKeops``).
